...
// The live-stream payload, re-exported for the frontend on the same reach and the
// same type-only terms as the analytics registry above.
//
// The generated `realtime.gen.ts` already imports `EventHubTopics` from this
// path, so the client's `subscribeToTopic('meetup-live', handler)` is typed —
// but a component that wants to NAME the event in a `switch` had nowhere to
// import it from. Without this the alternative is re-declaring the union in the
// browser, which is the one place a drift between what the server publishes and
// what the room renders would be invisible until the night.
export type {
  EventHubTopics,
  LiveQuestion,
  MeetupLiveEvent,
